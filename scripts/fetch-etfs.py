...
def fetch_benchmark_data(full_name, index_name):
    ...
    try:
        ...
        logger.info(f"Received {len(docs)} entries from benchmark data for index {index_name}")
        logger.debug(f"Benchmark response for index {index_name}: {json.dumps(data, indent=2)}")
        return docs
    except requests.RequestException as e:
        logger.error(f"Failed to fetch benchmark data for {full_name} (index {index_name}): {e}")
        return []
...
def fetch_tracking_data(ticker, index_name):
    ...
    try:
        ...
        data = response.json()
        if data is None:
            logger.info(f"Received null tracking data for {ticker} (index {index_name})")
            return {}
        logger.info(f"Received tracking data for {ticker} (index {index_name})")
        logger.debug(f"Tracking response for {ticker} (index {index_name}): {json.dumps(data, indent=2)}")
        return data
    except requests.RequestException as e:
        logger.error(f"Failed to fetch tracking data for {ticker} (index {index_name}): {e}")
        return {}
...
def fetch_fund_description(ticker, index_name):
    ...
    try:
        ...
        logger.debug(f"Description response for {ticker} (index {index_name}): {json.dumps(data, indent=2)}")
        return data
    except requests.RequestException as e:
        logger.error(f"Failed to fetch description for {ticker} (index {index_name}): {e}")
        return {}
...
def process_isin(isin_str, index_name, ticker):
    ...
    filtered_isins = [isin for isin in isins if not isin.startswith(('CA', 'JP', 'US', 'ZA', 'HK', 'NZ', 'KR', 'AU', 'XS'))]
    
    if len(filtered_isins) > 1:
        logger.error(f"Multiple valid ISINs found for {ticker} (index {index_name}): {filtered_isins}")
        print(f"Multiple valid ISINs found for {ticker} (index {index_name}): {filtered_isins}", file=sys.stderr)
        sys.exit(1)
    elif len(filtered_isins) == 0:
        logger.info(f"No valid ISINs after filtering for {ticker} (index {index_name}), skipping")
        return None
...
def process_csv(input_file, output_file):
    ...
    fieldnames = [
        'index-name', 'isin', 'share-name', 'currency-hedged', 'expense-ratio',
        'provider', 'replication-method', 'replication-model', 'dividend-policy-id',
        'creation-date', 'currency', 'size', 'tracking-error', 'tracking-difference',
        'description', 'trackinsight-ticker'
    ]
    ...
    for row in reader:
        ...
        if not benchmark_data and "STOXX" in search:
            benchmark_data = fetch_benchmark_data(search.replace("EURO", "Euro").replace("STOXX", "Stoxx"), index_name)

        if not benchmark_data and "EURO" in search:
            benchmark_data = fetch_benchmark_data(search.replace("EURO", "Euro"), index_name)

        if not benchmark_data:
            logger.warning(f"No benchmark data found for {row['full-name']} (index {index_name})")
            continue

        # Process all results from benchmark data
        for fund in benchmark_data:
            ticker = fund.get('ticker')
            logger.info(f"Processing fund: {ticker or 'unknown ticker'} (index {index_name})")
            ...
            description_data = fetch_fund_description(ticker, index_name) if ticker else {}
            ...
            output_row = {
                ...
                'replication-method': map_value(fund.get('replication_method', ''), REPLICATION_METHODS),
                'replication-model': map_value(fund.get('replication_model', ''), REPLICATION_MODELS),
                'dividend-policy-id': map_value(fund.get('dividendPolicyId', ''), DIVIDEND_POLICIES),
        ...
        logger.info(f"Completed processing {len(benchmark_data)} funds for index {index_name}")

def main():
    parser = argparse.ArgumentParser(description='Process index data from CSV and API')
    parser.add_argument('input', nargs='?', type=argparse.FileType('r'), 
                       default=sys.stdin, help='Input CSV file (default: stdin)')
